...
fn main() {
    ...
    println!("Tiempo secuencial: {:.4} segundos", duration_seq.as_secs_f64());
    ...
    println!("Tiempo paralelo:   {:.4} segundos", duration_par.as_secs_f64());

    // -------------------------------------------------------------------------
    // Verificación
    // -------------------------------------------------------------------------
    let valid = c_seq.iter().zip(c_par.iter()).all(|(row_s, row_p)| {
        row_s.iter().zip(row_p.iter()).all(|(val_s, val_p)| {
            (val_s - val_p).abs() < 1e-9
        })
    });
